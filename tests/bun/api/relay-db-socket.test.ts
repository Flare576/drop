...
async function renderConfig(env: RenderEnvironment): Promise<RenderedConfig> {
  const basename = `config.db-socket-test.${crypto.randomUUID()}.php`;
...
test("render-config emits DB_SOCKET so the documented unix-socket branch is actually deployable", async () => {
  const socketPath = "/var/run/mysqld/mysqld.sock";
  const env: RenderEnvironment = {
    DB_HOSTNAME: "tcp-host-should-not-win-when-socket-exists",
    DB_DATABASE: "drop_dev",
    DB_USERNAME: "drop_dev",
    DB_PASSWORD: "drop_dev",
    DATA_PATH: "/tmp/drop-data",
    DB_SOCKET: socketPath,
  };

  const { renderedSource, constants } = await renderConfig(env);

  expect(renderedSource).toContain("define('DB_SOCKET'");
  expect(constants.DB_SOCKET).toBe(socketPath);
});
